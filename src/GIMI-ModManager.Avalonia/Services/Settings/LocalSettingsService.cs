...
/// <summary>
/// Plain-JSON settings store, ported unchanged from the WinUI app. Uses Core's
/// <see cref="IFileService"/> and stores files under the platform's local-application-data
/// folder (Environment.SpecialFolder.LocalApplicationData → ~/.local/share on Linux).
/// This keeps the on-disk layout identical to the Windows build.
/// </summary>
public class LocalSettingsService : ILocalSettingsService
{
    private const string DefaultApplicationDataFolder = "ApplicationData";
    private const string DefaultLocalSettingsFileName = "LocalSettings.json";
    private const string DefaultAppScopedSettingsFileName = "LocalAppSettings.json";
    private const string Jasm = "JASM";
    ...
    private readonly string _appScopedSettingsFile = DefaultAppScopedSettingsFileName;
    ...
    public string AppScopedSettingsLocation => Path.Combine(_localApplicationData, Jasm, _appScopedSettingsFile);
    ...
    public LocalSettingsService(IFileService fileService)
    {
        ...
        _jasmApplicationDataFolder = Path.Combine(_localApplicationData, Jasm);
        _applicationDataFolder = Path.Combine(_jasmApplicationDataFolder, DefaultApplicationDataFolder);
        _localSettingsFile = DefaultLocalSettingsFileName;
    ...
    public void SetApplicationDataFolderName(string folderName)
    {
        ...
        _applicationDataFolder = Path.Combine(_localApplicationData, Jasm, folderName);
        ...
        where T : notnull
    {
        ...
        await Task.Run(() => _fileService.Save(folderPath, fileName, json, serializeContent: false))
